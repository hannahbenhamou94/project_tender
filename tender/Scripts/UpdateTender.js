...
function renderTender(tender) {
 ...
 // var f = new Date(parseInt(tender.hourFinish.substr(6)));
  ...
  //  alert(";;;;;;;" + tender.codCategory);
    if (tender.numType==1)
        $("#typeTender option[value='1']").prop('selected', true);
    else if (tender.numType == 2)
        $("#typeTender option[value='2']").prop('selected', true);
    else
        $("#typeTender option[value='3']").prop('selected', true);
    ...
    if (tender.status.trim() == "פעיל") {
     
        $("#statatus option[value='1']").prop('selected', true);
    }
    else if (tender.status.trim() == "סגור")
        $("#statatus option[value='2']").prop('selected', true);
    else
        $("#statatus option[value='3']").prop('selected', true);
